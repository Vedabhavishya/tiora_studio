...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-brand-light text-brand font-sans selection:bg-brand-accent/30">
      ...
      {offers.length > 0 && (
        <div className="w-full bg-[#FFFDF6] text-[#1B2A4A] h-12 flex items-center justify-center overflow-hidden border-b border-[#1B2A4A]/10 relative z-30 shadow-sm">
          ...
          >
            {isLoading ? (
              ...
              [...navItems, ...navItems].map((item, index) => (
                ...
                >
                  <div className="w-32 h-32 md:w-48 md:h-48 rounded-xl overflow-hidden border-2 border-[#1B2A4A]/30 shadow-lg transition-transform duration-500 group-hover:scale-105 group-hover:border-[#1B2A4A] group-hover:shadow-[#1B2A4A]/20 relative">
                    <img 
                  ...
                  <span className="mt-4 text-xs md:text-sm font-bold tracking-wide text-brand/80 group-hover:text-[#1B2A4A] transition-colors text-center w-full max-w-[12rem] break-words block">
                    {item.label}
                  </span>
                </Link>
              ))
